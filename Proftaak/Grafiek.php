...
foreach ($sensorData as $data) {
    if ($data['user_id'] == $userId && $data['created_at'] >= $today_start && $data['created_at'] <= $today_end) {
...
$json_data = [];
foreach ($sensorData as $data) {
    if ($data['user_id'] == $userId && $data['created_at'] >= $seven_days_ago) {
        // Haal de datum zonder tijd
        $date = date('Y-m-d', strtotime($data['created_at']));
        if (!isset($json_data[$date])) { 
            $json_data[$date] = 0;
        }
        $json_data[$date] += intval($data['weight']);
    }
}
...
}, array_keys($json_data), $json_data);
...
<body>
    ...
    <div class="DoelSettings" id="WelcomeBack">
        ...
        <div class="progress mb-4">
            ...
            <div class="progress-bar" role="progressbar" style="width: <?php echo min($percentage, 100); ?>%;"
                ...
                <?php echo floor($total_today); ?> ml / <?php echo $daily_water_need; ?> ml
            </div>
        </div>
    ...
    <script>
        ...
        var labels = data.map(row => `Hoeveelheid water: ${row.waterML} ml`); 
        var values = data.map(row => Math.min(row.waterML, <?php echo $daily_water_need; ?>)); 
        ...
        var mijnGrafiek = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    borderWidth: 1,
                    borderRadius: 10 // Voeg deze regel toe voor afgeronde hoeken
                },
                {
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(tooltipItem) {
                                return `Tijd: ${tooltipItem.label.split('\n')[0]}\nWaterML: ${tooltipItem.label.split('\n')[1].split(': ')[1]}`;
                            }
                        }
                    }
                }
